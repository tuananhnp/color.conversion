// https://observablehq.com/@tmcw/tables@444
...
- **options**:
  ...
  - **columns** \`Object\`: column-specific configuration, indexed by key:
    - \`[key]\`
      - **formatter** \`Function(value, rowIndex, row) ⇒ output\`: value formatter for cells
...
This is a table of the Titanic survivors dataset - it shows the data in natural order, first, but the columns are sortable. It shows usage of custom formatters for the first and last rows.`
)}
...
function _7(table,kenyaData,html,d3){return(
table(kenyaData, {
  columns: {
    method: {
      title: ''
...
function _10(name,md){return(
name && md`## ${name.name}

I was elected ${name.elected} and have an approval rating of ${name.approval}.`
)}

function _table(defaultOptions,html,style,th,identity){return(
(data, options) => {
  options = Object.assign({}, defaultOptions, options);
  const {sortable, rank} = options;
  ...
  function bake() {
    if (sortKey) {
      data = data.slice().sort((a, b) => {
        ...
        if (as == bs) return JSON.stringify(a) > JSON.stringify(b);
        let res = as > bs;
    ...
    return html`<div><div>
      <style>${style}</style>
        <table class='pretty-table ${options.style}'>
          ${options.header === false ? `` : html`<thead>
            ${rank ? html`<th></th>` : ''}
            ${columns.map(c => {
          ...
          <tbody>
            ${data.map((row, i) => html`<tr>
              ...
              ${columns.map(c => {
                let displayValue = (c.options.formatter || identity)(row[c.key], i, row);
...
(c, sortKey, sortDirection, sortable) => {
  ...
  console.log(c, sortKey, sortDirection, c.key);
  let arrow = html`<span style='${sortKey === c.key ? '' : 'visibility:hidden'}'>${sortIndicator}</span>`;
...
function _spotifyCharts(d3){return(
d3.csv('https://crossorigin.me/https://spotifycharts.com/regional/global/daily/latest/download')
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["table","kenyaData","html","d3"], _7);
  ...
  main.variable(observer()).define(["name","md"], _10);
  main.variable(observer("table")).define("table", ["defaultOptions","html","style","th","identity"], _table);
  main.variable(observer("th")).define("th", ["html"], _th);
  main.variable(observer("spotifyCharts")).define("spotifyCharts", ["d3"], _spotifyCharts);
